...
def test_unified_yaml_loader_parses_cortex_global_and_features():
        ...
        p.write_text(
            "\n".join(
                [
                    ...
                    "  parameter_loop_consolidation_interval: 3",
                    ...
                    "  qdt_qspin_live_activation: true",
                    "  qdt_qspin_live_kill_switch_enabled: true",
                    "  qdt_qspin_live_max_payload_tokens: 5",
                    "features:",
                    "  hgm_enabled: true",
    ...
    assert unified.cortex.enable_parameter_loop_auto_consolidation is True
    assert unified.cortex.parameter_loop_consolidation_interval == 3
    assert unified.cortex.parameter_loop_consolidation_max_bundles == 2
    ...
    assert unified.cortex.qdt_qspin_live_activation is True
    assert unified.cortex.qdt_qspin_live_kill_switch_enabled is True
    assert unified.cortex.qdt_qspin_live_max_payload_tokens == 5
...
def test_build_cortex_from_yaml_constructs_model_from_typed_entrypoint():
        ...
        p.write_text(
            "\n".join(
                [
                    ...
                    "  qdt_transformer_layers: 1",
                    "  qdt_qspin_live_activation: true",
                    "  qdt_qspin_live_max_payload_tokens: 3",
                    "  hgm_enabled: true",
                    "stores: {}",
                    "ahg: {}",
                    "distill: {}",
    ...
    assert model.describe_parameter_storage_loop()["auto_consolidation_enabled"] is True
    assert model.parameter_loop_consolidation_interval == 1
    ...
    assert fabric["qdt_config"]["qspin_live_activation"] is True
    assert fabric["qdt_config"]["qspin_live_mode"] == "experimental_live"
    assert fabric["qdt_config"]["qspin_live_max_payload_tokens"] == 3
